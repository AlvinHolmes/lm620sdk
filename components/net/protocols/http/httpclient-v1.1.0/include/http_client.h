...
#define CONFIG_HTTP_SECURE
#define CONFIG_HTTP_RECV_NON_BOLCK /* HTTP data recv is non block */
#define CONFIG_HTTP_FILE_OPERATE 1
#define CONFIG_HTTP_RECV_TIMER_WAKEUP 1

#ifndef CONFIG_HTTP_RECV_TIMEOUT_MS
#ifdef MIDDLEWARE_TEST_AT_HTTP_ON
#define CONFIG_HTTP_RECV_TIMEOUT_MS 1000
#else
#define CONFIG_HTTP_RECV_TIMEOUT_MS 5000
#ifdef CONFIG_HTTP_RECV_NON_BOLCK
#define CONFIG_HTTP_RECV_TIMEOUT_NOBLOCK_MS 1
#endif /* endif CONFIG_HTTP_RECV_NON_BOLCK */
#endif /* endif MIDDLEWARE_TEST_AT_HTTP_ON */
#endif /* endif CONFIG_HTTP_RECV_TIMEOUT_MS */
...
#endif
#ifdef CONFIG_HTTP_RECV_NON_BOLCK
    int recv_flags;                 /**< response data recv flags. block or not block */
    int is_response_stream;         /**< http response stream? if 1, is stream; if 0, not stream */
#endif
    int recv_timeout_ms;            /* response data recv timeout */
    int ssl_authmode;               /* ssl verify mode */
} http_client_t;
...
typedef struct {
    ...
    char *response_buf;          /**< buffer to store the response body data. 申请的内存大小时要比实际返回的响应体内容多 1 个字节, 用于存放 '\0' */
    ...
    int range_start;             /**< range start position in request header. */
    int range_len;               /**< range length in request header. */
} http_client_data_t;
...
HTTP_RESULT_CODE http_client_send_file(http_client_t *client, const char *url, int method, http_client_data_t *client_data, const char *filename);
