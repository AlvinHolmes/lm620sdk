...
static int http_get_sample_task_func(char argc, char **argv)
{
    ...
    ret = http_client_create(&client, URI);
    if(0 == ret)
    {
        ret = http_client_get(&client, (const char *)URI, &client_data);
        //OS_ASSERT(0 <= ret);
...
static int http_post_sample_task_func(char argc, char **argv)
{
...
static int http_put_sample_task_func(char argc, char **argv)
{
    char *content_type = "text/csv";
...
static int http_delete_sample_task_func(char argc, char **argv)
{
    http_client_t client = {0};
    http_client_data_t client_data = {0};
    char *buf = NULL;
...
static int http_get_file_task_func(char argc, char **argv)
{
    http_client_t client = {0};
    http_client_data_t client_data = {0};
    char *FILE_NAME = NULL;
