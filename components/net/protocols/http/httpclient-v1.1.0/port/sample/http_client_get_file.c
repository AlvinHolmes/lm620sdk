...
#define OS_ERROR    ERR_ARG
#define OS_EOK      ERR_OK
...
#define HTTP_REQUEST_BLOCK_SIZE 1024
...
static int http_get_file(const char* URI, const char* filename)
{
    ...
    if(NULL == last_file_name || (0 != strcmp(last_file_name, filename)))
    {
        ...
        HTTP_CLIENT_DBG_PRINTF("prepare to download new file:%s\r\n", filename);
    }
    else if(0 == strcmp(last_file_name, filename) && !download_finish_flag)
    {
        //LOG_D(HTTP_TAG, "resume from break point!\n");
        HTTP_CLIENT_DBG_PRINTF("resume from break point!\r\n");
    }
    else
    {
        block_number = 0;
        ...
        HTTP_CLIENT_DBG_PRINTF("prepare to download the same file:%s again!\r\n", filename);
    }
    buf = malloc(BUF_SIZE);
    if (buf == NULL)
    {
    ...
    do
    {
        ...
        //HTTP_CLIENT_DBG_PRINTF("the file total size is:%d\r\n", total_length);
        if(ret == 0 && (response_length > 0) &&(count_len < total_length))
        {
            ...
            HTTP_CLIENT_DBG_PRINTF("the file remaining size is:%d\r\n", left_length);
        }
            ...
            && ((total_length - count_len) != 0))
        {
            ...
            //HTTP_CLIENT_DBG_PRINTF("the last data size is:%d \r\n", last_data);
        }
    ...
    if(total_length == count_len && 0 == ret)
    {
        ...
        HTTP_CLIENT_DBG_PRINTF("the file:%s is downloaded!\r\n", filename);
    }
    else
    {
        ...
        download_finish_flag = 0;
        HTTP_CLIENT_DBG_PRINTF("the file:%s is not downloaded!\n", filename);
    }
...
void httpclient_get_file(char argc, char** argv)
{
    int ret = ERR_ARG;

    if (argc != 3)
    {
       HTTP_CLIENT_DBG_PRINTF("Please using: http_client_get_file <URI> <filename>");
       return;
    }

    ret = http_client_create((http_client_t *)argv[2], argv[1]);
    if(0 == ret)
    {
        http_get_file(argv[1], argv[2]);
    }
    http_client_close((http_client_t *)argv[2]);
    return;
}
...
//NR_SHELL_CMD_EXPORT(httpclient_get_file_sample, httpclient_get_file);
