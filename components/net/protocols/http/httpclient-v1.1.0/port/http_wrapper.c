...
#if CONFIG_HTTP_RECV_TIMER_WAKEUP
static void http_wakeup_timer_callback(void *argument)
{
    LOG_D(HTTP_TAG, "http timer wakeup");
    return;
}

static osTimer_t http_wakeup_timer_new(osTimerFunc_t callback, void *argument)
{
    LOG_D(HTTP_TAG, "http timer new");
    if(callback == NULL)
    {
        callback = http_wakeup_timer_callback;
    }
    return osTimerNew(callback, osTimerOnce, argument, NULL);
}

static osStatus_t http_wakeup_timer_start(osTimer_t timer, uint32_t timeout_ms, int tags, char* desc)
{
    LOG_D(HTTP_TAG, "http timer start [%p][%lu][%d][%s]", timer, timeout_ms, tags, ((desc == NULL) ? "" : desc));
    return osTimerStart(timer, osTickFromMs(timeout_ms)); /* osTimerStart 具有重置定时器的功能 */
}

static void http_wakeup_timer_free(osTimer_t timer, int tags)
{
    LOG_D(HTTP_TAG, "http timer free [%p][%d]", timer, tags);
    if(timer != NULL)
    {
...
static int8_t http_bind_pdp_id(int fd, uint8_t pdp_id)
{
    int ret = -1;
    struct netif *netif = NULL;
    struct ifreq optval = {0};

    if(pdp_id > NET_CID_MAX)
    {
        LOG_E(HTTP_TAG, "pdp id out of range [%d]", pdp_id);
        return HTTP_ERECV;
    }
    ...
    {
        netif = netif_get_by_cid(pdp_id);
    }

    if(NULL == netif)
    {
        LOG_E(HTTP_TAG, "bind netif is null");
        return HTTP_EPARSE;
    }

    if (!netif_index_to_name(netif_get_index(netif), optval.ifr_name))
    {
        LOG_E(HTTP_TAG, "bind index to name fail");
        return HTTP_EPROTO;
    }

    ret = lwip_setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, &optval, sizeof(optval));
    if (0 != ret)
    {
        LOG_E(HTTP_TAG, "bind pdp_id fail [%d]", ret);
        return HTTP_ECONN;
    }
 ...
 */
static int http_ssl_net_connect( mbedtls_net_context *ctx, const char *host,
                         const char *port, int proto, uint8_t pdp_id )
{
    int ret;
    ...
    for( cur = addr_list; cur != NULL; cur = cur->ai_next )
    {
        ...
        if(HTTP_SUCCESS == http_bind_pdp_id(ctx->fd, pdp_id))
        {
...
int http_tcp_conn_wrapper_extend(http_client_t *client, const char *host, uint8_t pdp_id)
{
    ...
    for ( cur = addr_list; cur != NULL; cur = cur->ai_next )
    {
        ...
        if(HTTP_SUCCESS == http_bind_pdp_id(client->socket, pdp_id))
        {
...
int http_tcp_conn_wrapper(http_client_t *client, const char *host)
{
    ...
    http_tcp_conn_wrapper_extend(client, host, 0);
    return ret;
}
...
int http_tcp_recv_wrapper(http_client_t *client, char *buf, int buflen, int timeout_ms, int *p_read_len)
{
    ...
    while (readLen < buflen)
    {
        ...
        if(H_RECV_NONBLOCK != optRecvFlags) /* http recv 阻塞方式时才需要创建唤醒定时器 */
        {
            if(NULL == wakeupTimer)
            {
                wakeupTimer = http_wakeup_timer_new(NULL, NULL);
                if(NULL == wakeupTimer)
                {
                    LOG_E(HTTP_TAG, "http tcp wakeup timer create fail");
                    return HTTP_ENOBUFS;
                }
            }

            timer_ret = http_wakeup_timer_start(wakeupTimer, timeout_ms, __LINE__, "HTTP_TCP_RECV");
            if(osOK != timer_ret)
            {
                LOG_E(HTTP_TAG, "http tcp wakeup timer start fail [%d]", timer_ret);
                http_wakeup_timer_free(wakeupTimer, __LINE__);
                wakeupTimer = NULL;
                return HTTP_ENOBUFS;
            }
        }
...
int http_ssl_conn_wrapper_extend(http_client_t *client, const char *host, uint8_t pdp_id)
{
    ...
    int optSslAuthMode = MBEDTLS_SSL_VERIFY_NONE;
    int optLen = sizeof(int);
    http_client_getopt(client, HO_SSL_AUTHMODE, &optLen, &optSslAuthMode);
    if(MBEDTLS_SSL_VERIFY_NONE == optSslAuthMode)
    {
        if (client->server_cert)
        {
            authmode = MBEDTLS_SSL_VERIFY_REQUIRED;
...
#ifdef MBEDTLS_X509_CRT_PARSE_C
        ret = mbedtls_x509_crt_parse(&ssl->clicert, (const unsigned char *)client->client_cert, client->client_cert_len + 1);
        if (ret < 0) {
    ...
    */
     ...
     */
    snprintf(port, sizeof(port), "%d", client->remote_port) ;
    if ((ret = http_ssl_net_connect(&ssl->net_ctx, host, port, MBEDTLS_NET_PROTO_TCP, pdp_id)) != 0) {
    ...
    if ((value = mbedtls_ssl_set_hostname(&ssl->ssl_ctx, host)) != 0) {
        LOG_E(HTTP_TAG, "mbedtls_ssl_set_hostname() failed, value:-0x%x.", -value);
        ret = -1;
        goto exit;
    }
...
int http_ssl_conn_wrapper(http_client_t *client, const char *host)
{
    ...
    http_ssl_conn_wrapper_extend(client, host, 0);
    return ret;
}
    ...
    while (readLen < buflen) {
        ...
        if(H_RECV_NONBLOCK != optRecvFlags) /* http recv 阻塞方式时才需要创建唤醒定时器 */
        {
            if(NULL == wakeupTimer)
            {
                ...
                if(NULL == wakeupTimer)
                {
                    LOG_E(HTTP_TAG, "http ssl wakeup timer create fail");
                    return HTTP_ENOBUFS;
                }
            }

            timer_ret = http_wakeup_timer_start(wakeupTimer, timeout_ms, __LINE__, "HTTP_SSL_RECV");
            if(osOK != timer_ret)
            {
                LOG_E(HTTP_TAG, "http ssl wakeup timer start fail [%d]", timer_ret);
                http_wakeup_timer_free(wakeupTimer, __LINE__);
                wakeupTimer = NULL;
                return HTTP_ENOBUFS;
            }
        ...
        if (ret == 0) {
            ...
            LOG_D(HTTP_TAG, "mbedtls_ssl_read, return:%d, readLen:%d", ret, readLen);
...
int http_tcp_recv_file_wrapper(http_client_t *client, char *buf, int buflen, int timeout_ms, int *p_read_len, const char *filename)
{
    ...
    LOG_D(HTTP_TAG, "http_get_sample: filename:%s --\n", filename);
    if (NULL == fp)
    {
        return ret;
    }
        ...
        if (ret > 0)
        {
            if(format_flag == 1)
            {
                if(buf_str == NULL)
                {
                    ...
                    fwrite(buf_str, 1, ret-(buf_str-buf), fp);
                }
                else
                {
                    readLen += ret;
...
int http_tcp_recv_wrapper_nonblock(http_client_t *client, char *buf, int recv_len, int *p_read_len)
{
    ...
    ret = recv(sockfd, buf, recv_len, MSG_DONTWAIT);
    if (ret < 0 )
    {
        LOG_E(HTTP_TAG, "http_tcp_recv_wrapper_nonblock recv fail[%d]", ret);
        return HTTP_ERECV;
    }

    *p_read_len = ret;
    LOG_D(HTTP_TAG, "http_tcp_recv_wrapper_nonblock recv ok[%d]", ret);
...
int http_ssl_recv_wrapper_nonblock(http_client_t *client, char *buf, int recv_len, int *p_read_len)
{
    ...
    if (!ssl)
    {
        LOG_E(HTTP_TAG, "http_ssl_recv_wrapper_nonblock ssl is null");
        return HTTP_ECONN;
    }

    mbedtls_ssl_conf_read_timeout(&ssl->ssl_conf, timeout_ms);

    ret = mbedtls_ssl_read(&ssl->ssl_ctx, (unsigned char *)buf, recv_len);
    if (ret < 0 )
    {
        LOG_E(HTTP_TAG, "http_ssl_recv_wrapper_nonblock recv fail[%d]", ret);
        return HTTP_ERECV;
    }

    *p_read_len = ret;
    LOG_D(HTTP_TAG, "http_ssl_recv_wrapper_nonblock recv ok[%d]", ret);
