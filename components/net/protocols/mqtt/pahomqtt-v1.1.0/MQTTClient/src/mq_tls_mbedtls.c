...
{                                                                        \
    ...
    mbedtls_strerror(errno, err_message, 33);                            \
                                                                         \
    MQTT_PRINT_ERROR(MQ_TLS_TAG"%s, mbedtls error: err_code=%d, err_message=%s.\r\n", \
          msg_head,                                                      \
          errno,                                                         \
          err_message);                                                  \
}
...
static void _log_mbedtls_debug(void *ctx, int level, const char *file, int line, const char *str)
{
    ...
    MQTT_PRINT_DEBUG(MQ_TLS_TAG"%s:%04d: %s\r\n", file, line, str);
}
#endif
...
static int tls_network_mbedtls_init(mq_tls_session_t *tls_session)
{
    ...
    {
        MQTT_PRINT_INFO(MQ_TLS_TAG"TLS network mbedtls initialized.\r\n");
    }
...
static int tls_network_mbedtls_cleanup(mq_tls_session_t *tls_session)
{
    ...
    MQTT_PRINT_INFO(MQ_TLS_TAG"TLS network mbedtls library cleanup done.\r\n");
                                       ...
                                       const char *client_pwd)
{
    ...
    if (mbedtls_err != 0)
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"Failed to read credentials, ret=0x%x\r\n", mbedtls_err);
        goto exit;
    }
...
static int tls_network_mbedtls_handshake(mq_tls_session_t *tls_session)
{
    ...
    MQTT_PRINT_INFO(MQ_TLS_TAG"Start handshake tick:%d\r\n", osTickGet());
    ...
    MQTT_PRINT_INFO(MQ_TLS_TAG"Finish handshake tick:%d\r\n", osTickGet());
    ...
    if (verify_result != 0)
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"Failed to verify server certificate, result %u.\r\n", verify_result);
        char verify_info[128] = {0};
        mbedtls_x509_crt_verify_info(verify_info, 128, "  ! ", verify_result);
        MQTT_PRINT_ERROR(MQ_TLS_TAG"Server certificate verification info: %s\r\n", verify_info);

        return -1;
    }
    else
    {
        MQTT_PRINT_INFO(MQ_TLS_TAG"Server certificate verified success.\r\n");
    }
#endif
                     ...
                     const char *client_pwd)
{
...
#ifdef OS_USING_SHELL
    // MQTT_PRINT_INFO(MQ_TLS_TAG"Memory usage before TLS connection is established:\r\n");
    // sh_exec("show_mem");
#endif
    ...
    if (mbedtls_err != 0)
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"Failed to init mbedtls library.\r\n");
        goto cleanup;
    }
    MQTT_PRINT_INFO(MQ_TLS_TAG"Init mbedtls library ok.\r\n");

    mbedtls_err = tls_network_mbedtls_context(tls_session, root_ca, client_cert, client_key, client_pwd);
    if (mbedtls_err != 0)
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"Failed to set mbedtls context.\r\n");
        goto cleanup;
    }
    MQTT_PRINT_INFO(MQ_TLS_TAG"set mbedtls context ok.\r\n");

    mbedtls_err = tls_network_mbedtls_handshake(tls_session);
    if (mbedtls_err != 0)
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"Failed to finish mbedtls handshake.\r\n");
        goto cleanup;
    }

#ifdef OS_USING_SHELL
    // MQTT_PRINT_INFO(MQ_TLS_TAG"Memory usage after TLS connection is established:\r\n");
    // sh_exec("show_mem");
#endif
                                     ...
                                     const char *client_pwd)
{
    ...
    if (tls_session == NULL)
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"No memory for MbedTLS session object.\r\n");
        return ret;
    }
    ...
    if (mbedtls_err != 0)
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"Mbedtls fail to establish tls connection, return -0x%x\r\n", -mbedtls_err);
        goto exit;
    }

    MQTT_PRINT_INFO(MQ_TLS_TAG"MbedTLS connect successed \r\n");
...
exit:
    ...
    MQTT_PRINT_ERROR(MQ_TLS_TAG"MbedTLS connection establish failed.\r\n");
...
int mq_tls_network_mbedtls_close(mq_tls_session_t **pass_tls_session)
{
    ...
    if (NULL == tls_session || (uintptr_t)tls_session == (uintptr_t)-1)
    {
        return -1;
    }
    ...
    {
        MQTT_PRINT_INFO(MQ_TLS_TAG"TLS connection closed.\r\n");
    }*/
    ...
    MQTT_PRINT_INFO(MQ_TLS_TAG"MbedTLS connection close success, mbedtls library cleanup done.\r\n");
...
int mq_tls_network_mbedtls_read(mq_tls_session_t *tls_session, unsigned char *buf, size_t len)
{
    ...
    if (NULL == tls_session || (uintptr_t)tls_session == (uintptr_t)-1 ||  NULL == buf)
    {
        return -1;
    }
    ...
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"MbedTLS read data error, return -0x%x\r\n", -ret);
    }
...
int mq_tls_network_mbedtls_write(mq_tls_session_t *tls_session, const unsigned char *buf , size_t len)
{
    ...
    {
        MQTT_PRINT_ERROR(MQ_TLS_TAG"MbedTLS write data error, return -0x%x\r\n", -ret);
    }
