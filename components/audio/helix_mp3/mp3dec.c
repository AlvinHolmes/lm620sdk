...
void MP3GetID3TagSize(unsigned char *file, unsigned int fileSize, unsigned int *tagSize)
{
    ID3Header *id3Header = (ID3Header *)file;

    *tagSize = 0;

    if(memcmp(id3Header->id3_identifier,"ID3", 3) == 0)
    {
        *tagSize = (id3Header->id3_size[0] & 0x7F) * 0x200000 + ((id3Header->id3_size[1] & 0x7F) * 0x4000) + 
                    ((id3Header->id3_size[2] & 0x7F) * 0x80) + (id3Header->id3_size[3] & 0x7F);
        *tagSize += sizeof(ID3Header);
