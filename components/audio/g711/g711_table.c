...
#else
/* 16384 entries per table (16 bit) */
unsigned char *linear_to_alaw = NULL;
unsigned char *linear_to_ulaw = NULL;

/* 16384 entries per table (8 bit) */
unsigned short *alaw_to_linear = NULL;
unsigned short *ulaw_to_linear = NULL;
#endif
...
{
    if(NULL == linear_to_alaw)
    {
        linear_to_alaw = osMalloc(65536);
        OS_ASSERT(linear_to_alaw);
    }
...
{
    if(NULL == linear_to_ulaw)
    {
        linear_to_ulaw = osMalloc(65536);
        OS_ASSERT(linear_to_ulaw);
    }
...
{
    if(NULL == alaw_to_linear)
    {
        alaw_to_linear = (unsigned short *)osMalloc(256 * 2);
        OS_ASSERT(alaw_to_linear);
    }
...
{
    if(NULL == ulaw_to_linear)
    {
        ulaw_to_linear = (unsigned short *)osMalloc(256 * 2);
        OS_ASSERT(ulaw_to_linear);
    }
...
void ulaw_pcm16_tabledeinit()
{
    if(ulaw_to_linear)
    {
        osFree(ulaw_to_linear);
        ulaw_to_linear = NULL;
    }
}

// // PCM --> PCMA/PCMU
// int8_t PCM_Encode(uint8_t *pcmData, uint32_t pcmDataSize, PCM_Type encodeType, uint8_t *encodeData, uint32_t *encodeSize)
// {
//     if(pcmData == NULL || pcmDataSize == 0 || encodeData == NULL || 
//         encodeSize == NULL || (*encodeSize) < pcmDataSize || encodeType > PCM_UALW)
//     {
//         os_kprintf("PCM_Encode para error \r\n");
//         return -1;
//     }

//     uint8_t *tmpDataBuf = osMalloc(65536);
//     OS_ASSERT(tmpDataBuf);

//     if(encodeType == PCM_ALAW)
//     {
//         linear_to_alaw = tmpDataBuf;
//         pcm16_alaw_tableinit();
//         pcm16_to_alaw(pcmDataSize, (uint8_t *)pcmData, (uint8_t *)encodeData);
//         *encodeSize = (pcmDataSize >> 1);
//     }
//     else if( encodeType == PCM_ULAW)
//     {
//         linear_to_ulaw = tmpDataBuf;
//         pcm16_ulaw_tableinit();
//         pcm16_to_ulaw(pcmDataSize, (uint8_t *)pcmData, (uint8_t *)encodeData);
//         *encodeSize = (pcmDataSize >> 1);
//     }
...
// //PCMU/PCMA --> PCM
// int8_t PCM_Decode(uint8_t *pcmData, uint32_t pcmDataSize, PCM_Type encodeType, uint8_t *encodeData, uint32_t encodeSize)
// {
