	  ...
	  )
{
	...
	*ser_size = nb_of_bits[*mode];
	...
	for (i = 0; i < L_FRAME; i++)
	{
        max = __RV_SMAX16(abs_s(wsp[i]), max);
	}
    tmp = __RV_SMAX16(max, st->old_wsp_max);
#else
	for (i = 0; i < L_FRAME; i++)
	{
		tmp = abs_s(wsp[i]);
    ...
    #ifdef RISCV_DSP
        L_tmp = (Word32)__RV_KSLRAW(L_gain_code, Q_new);
    #else
    ...
    #ifdef RISCV_DSP
        L_tmp = (Word32)__RV_KSLRAW(L_tmp, (5 + shift));
	#else
    ...
    #ifdef RISCV_DSP
        L_tmp = (Word32)__RV_KSLRAW(L_tmp, (1 - shift));
    #else
...
int AMR_Enc_Encode(HAMRENC hCodec)
{
	...
	for (i = 0; i < L_FRAME16k; i++)   /* Delete the 2 LSBs (14-bit input) */
	{
		*(signal + i) = (Word16) (*(signal + i) & 0xfffC);
        ...
        if(reset_flag)
            reset_flag = !(Word16) (signal[i] ^ EHF_MASK);
	}
