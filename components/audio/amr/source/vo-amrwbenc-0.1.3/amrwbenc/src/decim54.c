		...
		)
{
    ...
    {
        y = (Word16 *)(fir_down1);
        x = sig - NB_COEF_DOWN + 1 + times; 
        s0 = 0;
        for(Word32 i = 0; i < 30; i++)
        {
            s0 += x[i] * y[i];
        }

        s1 = s0 + x[30] * y[30] - x[0] * y[0];
        s2 = s1 + x[31] * y[31] - x[1] * y[1];
        s3 = s2 + x[32] * y[32] - x[2] * y[2];

        tmpS = L_shl2(s0, 2);
		sig_d[j++] = extract_h(L_add(tmpS, 0x8000));
        tmpS = L_shl2(s1, 2);
        sig_d[j++] = extract_h(L_add(tmpS, 0x8000));
        tmpS = L_shl2(s2, 2);
        sig_d[j++] = extract_h(L_add(tmpS, 0x8000));
