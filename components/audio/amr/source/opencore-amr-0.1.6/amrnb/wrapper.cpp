...
void Decoder_Interface_Decode(void* state, const unsigned char* in, short* out, int bfi) {
	...
	*/
	...
	{
		type = in[0] & 0x0f;
		AMRDecode(state, (enum Frame_Type_3GPP) type, (UWord8*) in, out, IF2);
	}
...
int Encoder_Interface_Encode(void* s, int mode, const short* speech, unsigned char* out, int forceSpeech) {
	struct encoder_state* state = (struct encoder_state*) s;
	...
	if(forceSpeech == 1)
	{
		ret = AMREncode(state->encCtx, state->pidSyncCtx, (Mode)mode, (Word16*) speech, out, &frame_type, AMR_TX_IF2);
	}
	else if(forceSpeech == 2)
	{
		ret = AMREncode(state->encCtx, state->pidSyncCtx, (Mode)mode, (Word16*) speech, out, &frame_type, AMR_TX_IETF);
		out[0] |= 0x04;
	}
