...
typedef struct
{
    ...
    uint16_t lac;         /**暂不支持 位置区码*/
    uint32_t ci;          /**暂不支持 小区识别码*/
    uint8_t act;          /**暂不支持 网络访问技术*/
    uint8_t rac;          /**暂不支持 路由区域编码*/
    uint8_t cause_type;   /**暂不支持 reject_cause类型*/
    uint8_t reject_cause; /**暂不支持 注册失败原因*/
    uint8_t active_time;  /**暂不支持 T3324值*/
    uint8_t periodic_tau; /**暂不支持 T3412值*/
} cm_cereg_state_t;
