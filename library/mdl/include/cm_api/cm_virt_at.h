...
typedef struct {
	int32_t event;          /*!< 触发事件消息:实际会返回 CM_VIRT_AT_OK / CM_VIRT_AT_TOUT */
 ...
 *          补充说明：当前实现内部响应暂存缓冲区固定为 2048 字节；超出部分会被截断。
 */
int32_t cm_virt_at_send_sync(const uint8_t *at_cmd, uint8_t *rsp, int32_t *rsp_len, uint32_t timeout);

#if 1 /* 扩展接口，谨慎使用 */
/**
 * @brief 设置虚拟AT异步请求全局超时时间
 *
 * @param [in] timeout 异步请求超时时间，单位tick，必须大于0
 *
 * @return
 *   = 0  - 成功 \n
 *   < 0  - 失败
 *
 * @details `cm_virt_at_send_async()` 会使用这里设置的默认超时时间。
 *          如需支持超长命令，建议业务在调用前临时调大，完成后恢复。
 */
int32_t cm_virt_at_set_timeout(uint32_t timeout);
#endif /* 扩展接口，谨慎使用 */
